...
const payloadMaxDepth = Math.max(
	payloadDefaultDepth,
	toNonNegativeInt(process.env.PAYLOAD_API_MAX_DEPTH, DEFAULT_API_MAX_DEPTH),
)
const payloadGraphQLMaxComplexity = toPositiveInt(
	process.env.PAYLOAD_GRAPHQL_MAX_COMPLEXITY,
		...
		queries: (GraphQL, graphQLContext) => {
			...
			return {
				availableTagsByMicroPostType: {
					type: new GraphQL.GraphQLNonNull(new GraphQL.GraphQLList(new GraphQL.GraphQLNonNull(tagType))),
					args: {
						locale: {
					...
					resolve: async (
						...
						const tagIds = Array.from(
							new Set(microPostsResult.docs.flatMap(microPost => microPost.tags?.map(toTagId) ?? [])),
						)
