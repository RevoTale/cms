...
import { and, eq, isNull, or, sql } from '@payloadcms/db-postgres/drizzle'
...
const getCountOfNonTranslatedPosts = async (): Promise<CountResponse> => {
	...
	try {
		...
		const rows = await payload.db.drizzle
			...
			.leftJoin(
				micro_posts_locales,
				and(eq(micro_posts_locales._parentID, micro_posts.id), eq(micro_posts_locales._locale, 'es-ES')),
			)
