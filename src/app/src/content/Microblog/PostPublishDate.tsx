...
const PostPublishDate: FunctionComponent<Props> = ({ post, className, showTime = false, locale }) => {
	const unmaskedPost = getFragmentData(postPublishDate, post)
	const classNameOver = cn(linkClassName(), 'text-sm text-muted-foreground block break-words font-normal', className)
	...
	return (
		<NextLink className={classNameOver} href={getMicropostHref(unmaskedPost).asString()} locale={locale}>
			{publishedAt === null
				? 'not published'
