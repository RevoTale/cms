...
const BlogListItemAuthor: FunctionComponent<Props> = ({ author, className, locale, gap = true }) => {
	...
	return (
		<i className={cn('flex items-center justify-left gap-3 flex-wrap', className)}>
			{authorData?.avatar && authorData.slug ? (
				<NextLink locale={locale} className={imageClassName} href={getAuthorHref(authorData).asString()}>
					<ContentfulImageFill alt={abbrName(authorData.name ?? '')} asset={authorData.avatar} sizes="3rem" />
				</NextLink>
			) : (
				<InlineSkeleton className={imageClassName} />
			)}
			{authorData?.slug && authorData.name !== null ? (
				<NextLink
					locale={locale}
