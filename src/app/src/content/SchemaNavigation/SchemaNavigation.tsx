	...
	return (
		<>
			{branches.map((branch, index) => (
				<Fragment key={branch.href instanceof URL ? branch.href.toString() : branch.href}>
					<div
						className={cn(
					...
					>
								...
								{branches.length > 1 ? (
									<div
										className={cn(
											...
											index === 0 ? 'bottom-0 h-[50%]' : index === branches.length - 1 ? 'top-0 h-[50%]' : 'h-full',
										)}
									></div>
								) : null}
								<div className="w-1" />
								<div className={cn('w-7 h-1 bg-secondary relative')}>
						...
						<div className="py-3">
							<LocaleLink
								...
								key={branch.href instanceof URL ? branch.href.toString() : branch.href}
							>
								{branch.name}
							</LocaleLink>
						</div>
						{branch.children ? (
							<>
								...
								<div className="flex flex-col">
									<Branches locale={locale} branches={branch.children} />
								</div>
							</>
						) : null}
					</div>
