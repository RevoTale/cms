...
const transformLink = (href: string, translateLinks: Record<string, string>): string => {
	let trunc = href
...
const PostMarkdown: FunctionComponent<Props> = ({ markdown, translateLinks = {}, size = 'large', locale, rootUrl }) => {
	...
	const components: Partial<Components> = {
			...
			return (
				<figure className="relative ">
					<figcaption className="flex items-center rounded-t-lg justify-between px-4 py-2 bg-muted">
						...
						{typeof code === 'string' ? (
							<CopyButtonWithSkeleton text={code} className={cn('z-1', 'inline-flex', 'px-0')} variant="ghost" />
						) : null}
					</figcaption>
		...
		h1: ({ children }) => (
			<h2 className={cn('leading-tight text-2xl font-bold mb-2 mt-14', size === 'small' ? 'text-xl' : 'text-2xl')}>
				{children}
			</h2>
		),
		h2: ({ children }) => (
			<h3 className={cn('leading-tight text-xl font-semibold mt-12 -mb-2', size === 'small' ? 'text-lg' : 'text-xl')}>
				{children}
			</h3>
		),
		h3: ({ children }) => <h4 className="leading-tight text-xl font-bold mt-12 -mb-2">{children}</h4>,
		h4: ({ children }) => <h5 className="leading-tight text-xl font-bold mt-8 -mb-2">{children}</h5>,
		h5: ({ children }) => <h6 className="leading-tight text-xl font-bold mt-8 -mb-2">{children}</h6>,
		...
		>
			<Markdown
				...
				remarkPlugins={[
					...
					() => {
						return tree => {
							visit(tree, 'link', (node: Node & { url: string | undefined }) => {
								if (typeof node.url === 'string') {
									// eslint-disable-next-line no-param-reassign -- Intended to modify the node
									node.url = transformLink(node.url, translateLinks)
								}
							})
						}
					},
