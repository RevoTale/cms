					...
					<li key={doc.id}>
						<div className="flex flex-col">
							...
							<span className="text-sm">
								<PostPublishDate className="inline italic" locale={locale} post={doc} />{' '}
								<span className="text-muted-foreground font-medium">{t('by')}</span>{' '}
								{doc.authors?.map(author => (
									<LocaleLink
										locale={locale}
										className={linkClassName('font-medium text-muted-foreground')}
										key={author.id}
