...
const ensureTag = async (payload: Payload, locale: TypedLocale, tagSeed: TagSeed): Promise<string> => {
...
const ensureExternalLink = async (payload: Payload, externalLinkSeed: ExternalLinkSeed): Promise<string> => {
...
const resolveAuthorId = async ({
	...
	const safeAuthorName =
		trimmedUserName.length > 0 ? trimmedUserName.slice(0, AUTHOR_NAME_MAX_LENGTH) : fallbackAuthorName
