...
const getRandom = (
	values: RecordValue[],
	alreadyUsed: RecordValue[]
): null | RecordValue => {
	const filteredUsed = values.filter(
		a => !alreadyUsed.some(usedItem => usedItem.index === a.index)
	)
	if (filteredUsed.length === 0) {
		return null
	}
	const result =
		filteredUsed[Math.floor(Math.random() * filteredUsed.length)] ?? null
	if (result === null) {
...
const reducer = (state: State, action: Action): State => {
	switch (action.type) {
		...
		case 'SCROLL': {
			const nextWheelValues = getNextWheelValues(
				state.wheelValues,
				action.payload.values
			)
			const winner = action.payload.last
				? (nextWheelValues[Math.floor(nextWheelValues.length / half)]
						?.value ?? null)
				: state.winner
			return {
				...state,
...
const itemCount = 5
const half = 2
const RandomizerWheel: FunctionComponent<Props> = ({
	values,
	wheelDuration,
	disableWinnerDialog,
}) => {
	...
	const startScrollWheel = (): void => {
		...
		const makeNextTimeout = (): void => {
			...
			const nextIntervalTime =
				phases[
					Math.floor(phases.length / (wheelDuration / totalDuration))
				] ?? faultDuration
			...
			setTimeout(() => {
				scrollWheelDown(
					totalDuration + nextIntervalTime >= wheelDuration
				)
				makeNextTimeout()
			}, nextIntervalTime)
		}
		...
		<div className="flex flex-col gap-2 max-w-64 w-full">
			<div className="flex flex-col">
				{wheelValues.map((value, index) => {
					...
					return (
						<div
							className={cn(
								'flex gap-2 w-full rounded-lg p-1',
								isActive ? 'bg-secondary/80' : null
							)}
							key={value.index}>
							<div
								className={cn(
			...
			<Dialog
				...
				<DialogContent>
					...
					<div className="text-2xl font-semibold text-center underline underline-offset-4 break-all">
						{winner}
					</div>
			...
			{winner === null ? null : (
				<div>
					<div className="text-base font-medium text-center">
						We have a have winner!🎉
					</div>
					<div className="text-xl font-semibold p-3 break-all underline underline-offset-4 text-center">
						{winner}
					</div>
				</div>
			)}
