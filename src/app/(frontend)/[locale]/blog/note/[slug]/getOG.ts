...
`)
const getOG = (
	post: FragmentType<typeof OGPostFragment>,
	locale: string
): OpenGraph => {
	const {authors, meta, tags, publishedAt, title, attachment} =
		getFragmentData(OGPostFragment, post)
	const metaImage = meta?.image
	return {
		authors:
			authors
				?.map(author => getUrl(getAuthorHref(author), locale))
				.toString() ?? [],
		type: 'article',
		images: metaImage
			? getOGImage(metaImage)
			: attachment
				? getOGImage(attachment)
				: undefined,
