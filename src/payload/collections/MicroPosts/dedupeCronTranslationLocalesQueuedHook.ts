...
const dedupeCronTranslationLocalesQueuedHook: CollectionBeforeChangeHook<MicroPostData> = ({ data }) => {
	...
	for (const entry of data.cronTranslationLocalesQueued as CronLocale[]) {
		const locale =
			typeof entry === 'string' ? (isTypedLocale(entry) ? entry : null) : isSelectOption(entry) ? entry.value : null
