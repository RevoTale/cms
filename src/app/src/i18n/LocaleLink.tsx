...
const clearHref = (href: string): string => {
	...
	return nonLocalizedPart.startsWith('/')
		? nonLocalizedPart
		: `/${nonLocalizedPart}`
}
...
const LocaleLink = ({
	href,
	locale: promisedLocale,
	scroll,
	...props
}: ServerLinkProps): ReactNode => {
	const locale =
		typeof promisedLocale === 'string'
			? promisedLocale
			: use(promisedLocale)
	...
	if (href.startsWith('/')) {
		...
		const currentLocale: Locale =
			routing.locales.find((i): i is Locale => i === currentLocaleStr) ??
			routing.defaultLocale
		const cleanedHref = routing.locales.some(i => i === currentLocaleStr)
			? clearHref(href)
			: href
