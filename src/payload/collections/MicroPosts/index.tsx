...
const getPostTypeFromContent = (content: unknown): 'short' | 'long' => {
	if (typeof content !== 'string') {
		...
		const hasLongVariant = localizedValues.some(
			localizedValue => typeof localizedValue === 'string' && localizedValue.length >= SHORT_POST_MAX,
		)
...
const formatMicroPostSlug: FieldHook<MicroPostData, string | undefined> = ({ data, originalDoc, value }) => {
	if (typeof value === 'string' && value.trim().length > 0) {
...
const derivePostTypeBeforeValidateHook: CollectionBeforeValidateHook<MicroPostData> = ({ data }) => {
	if (!data) {
		return data
	}
